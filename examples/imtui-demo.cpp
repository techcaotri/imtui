...
void ImTui::ShowDemoWindow(bool* p_open)
{
    ...
    if (show_app_style_editor)        { 
        ImGui::SetNextWindowPos(ImVec2(80, 20), ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(80, 40), ImGuiCond_FirstUseEver);
        ImGui::Begin("Style Editor", &show_app_style_editor);
        ImTui::ShowStyleEditor();
        ImGui::End();
    }
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Text"))
    {
        ...
        if (ImGui::TreeNode("Word Wrapping"))
        {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(pos.x + wrap_width, pos.y), ImVec2(pos.x + wrap_width + 1, pos.y + ImGui::GetTextLineHeight()), IM_COL32(255,0,255,255));
    ...
    if (false && ImGui::TreeNode("Images"))
    {
        ImGuiIO& io = ImGui::GetIO();
    ...
    if (ImGui::TreeNode("Selectables"))
    {
        ...
        if (ImGui::TreeNode("Grid"))
        {
            ...
            {
                ImGui::PushID(i);
                if (ImGui::Selectable("Sailor", &selected[i], 0, ImVec2(8,4)))
                {
        ...
        if (ImGui::TreeNode("Alignment"))
        {
            ...
            for (int y = 0; y < 3; y++)
            {
                for (int x = 0; x < 3; x++)
                {
                    ...
                    ImGui::Selectable(name, &selected[3*y+x], ImGuiSelectableFlags_None, ImVec2(8,4));
                    ImGui::PopStyleVar();
                }
            }
            ImGui::TreePop();
    ...
    if (ImGui::TreeNode("Plots Widgets"))
    {
        ...
        static float arr[] = {
            ...
            0.2f, 0.7f, 1.5f, 0.5f, 0.32f, 0.3f, 0.2f,
            0.3f, 0.3f, 1.3f, 0.5f, 0.92f, 0.5f, 0.2f,
        };
        ...
        ImGui::Text("%s", "");
        ImGui::PlotHistogram("Histogram", arr, IM_ARRAYSIZE(arr), 0, NULL, 0.0f, 1.0f, ImVec2(0,16));
        ...
        ImGui::Text("%s", "");
        ImGui::PlotLines("Lines", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0,12));
        ImGui::Text("%s", "");
        ImGui::PlotHistogram("Histogram", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0,12));
        ImGui::Separator();
    ...
    {
        ImGui::PushItemWidth(0.7*ImGui::GetWindowWidth());
        ...
        ImGui::DragScalar("drag float ^2",  ImGuiDataType_Float,  &f32_v, 0.005f,  &f32_zero, &f32_one, "%f", ImGuiSliderFlags_Logarithmic); ImGui::SameLine(); HelpMarker("You can use the 'power' parameter to increase tweaking precision on one side of the range.");
        ImGui::DragScalar("drag double",    ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, NULL,     "%.10f grams");
        ImGui::DragScalar("drag double ^2", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, &f64_one, "0 < %.10f < 1", ImGuiSliderFlags_Logarithmic);
        ...
        ImGui::SliderScalar("slider float low^2", ImGuiDataType_Float,  &f32_v, &f32_zero, &f32_one,  "%.10f", ImGuiSliderFlags_Logarithmic);
        ImGui::SliderScalar("slider float high",  ImGuiDataType_Float,  &f32_v, &f32_lo_a, &f32_hi_a, "%e");
        ImGui::SliderScalar("slider double low",  ImGuiDataType_Double, &f64_v, &f64_zero, &f64_one,  "%.10f grams" );
        ImGui::SliderScalar("slider double low^2",ImGuiDataType_Double, &f64_v, &f64_zero, &f64_one,  "%.10f", ImGuiSliderFlags_Logarithmic);
    ...
    if (ImGui::TreeNode("Drag and Drop"))
    {
        ...
        if (ImGui::TreeNode("Drag and drop to copy/swap items"))
        {
            ...
            for (int n = 0; n < IM_ARRAYSIZE(names); n++)
            {
                ...
                if ((n % 3) != 0)
                    ImGui::SameLine();
                ImGui::Button(names[n], ImVec2(12,6));
...
static void ShowDemoWindowMisc()
{
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus"))
    {
        ...
        if (ImGui::TreeNode("Keyboard, Mouse & Navigation State"))
        {
            ...
            const char* mouse_button_names[] = { "Left", "Right", "Middle", "X1", "X2" };
            ImGui::Text("Mouse down:");     for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) if (io.MouseDownDuration[i] >= 0.0f)   { ImGui::SameLine(); ImGui::Text("%s (%.02f secs)", (i < IM_ARRAYSIZE(mouse_button_names)) ? mouse_button_names[i] : "Unknown", io.MouseDownDuration[i]); }
            ImGui::Text("Mouse clicked:");  for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) if (ImGui::IsMouseClicked(i))          { ImGui::SameLine(); ImGui::Text("%s", (i < IM_ARRAYSIZE(mouse_button_names)) ? mouse_button_names[i] : "Unknown"); }
            ImGui::Text("Mouse dbl-clicked:"); for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) if (ImGui::IsMouseDoubleClicked(i)) { ImGui::SameLine(); ImGui::Text("%s", (i < IM_ARRAYSIZE(mouse_button_names)) ? mouse_button_names[i] : "Unknown"); }
            ImGui::Text("Mouse released:"); for (int i = 0; i < IM_ARRAYSIZE(io.MouseDown); i++) if (ImGui::IsMouseReleased(i))         { ImGui::SameLine(); ImGui::Text("%s", (i < IM_ARRAYSIZE(mouse_button_names)) ? mouse_button_names[i] : "Unknown"); }
...
void ImTui::ShowAboutWindow(bool* p_open)
{
...
static void ShowExampleMenuFile()
{
    ...
    if (ImGui::BeginMenu("Options"))
    {
        ...
        ImGui::BeginChild("child", ImVec2(0, 8), true);
        for (int i = 0; i < 10; i++)
...
static void ShowExampleAppLayout(bool* p_open)
{
    ...
    if (ImGui::Begin("Example: Simple layout", p_open, ImGuiWindowFlags_MenuBar))
    {
        ...
        ImGui::BeginChild("left pane", ImVec2(30, 0), true);
        for (int i = 0; i < 100; i++)
        {
            char label[128];
...
static void ShowExampleAppConstrainedResize(bool* p_open)
{
    ...
    ImGui::SetNextWindowPos(ImVec2(20, 14), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(ImVec2(80, 20), ImGuiCond_FirstUseEver);
    if (ImGui::Begin("Example: Constrained Resize", p_open, flags))
    {
        ...
        if (ImGui::Button("20x10")) { ImGui::SetWindowSize(ImVec2(20, 10)); } ImGui::SameLine();
        if (ImGui::Button("50x25")) { ImGui::SetWindowSize(ImVec2(50, 25)); } ImGui::SameLine();
        if (ImGui::Button("80x20")) { ImGui::SetWindowSize(ImVec2(80, 20)); }
...
static void ShowExampleAppWindowTitles(bool*)
{
    ...
    ImGui::SetNextWindowPos(ImVec2(30, 20), ImGuiCond_FirstUseEver);
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ImGui::SetNextWindowPos(ImVec2(10, 4), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(ImVec2(180, 50), ImGuiCond_FirstUseEver);
    ...
    if (ImGui::BeginTabBar("##TabBar"))
    {
        ...
        if (ImGui::BeginTabItem("BG/FG draw lists"))
        {
            ...
            if (draw_bg)
                ImGui::GetBackgroundDrawList()->AddCircle(window_center, window_size.x * 0.3f, IM_COL32(255, 0, 0, 200), 48, 1);
            if (draw_fg)
                ImGui::GetForegroundDrawList()->AddCircle(window_center, window_size.y * 0.20f, IM_COL32(0, 255, 0, 200), 48, 1);
            ImGui::EndTabItem();
        }
    ...
    if (!close_queue.empty())
    {
        ...
        else
        {
            ...
            if (ImGui::BeginPopupModal("Save?"))
            {
                ...
                if (ImGui::BeginListBox("##", ImVec2(-FLT_MIN, close_queue_unsaved_documents)))
                {
                    for (int n = 0; n < close_queue.Size; n++)
                        if (close_queue[n]->Dirty)
...
void ImTui::ShowAboutWindow(bool*) {}
void ImTui::ShowDemoWindow(bool*) {}
void ImTui::ShowUserGuide() {}
void ImTui::ShowStyleEditor(ImGuiStyle*) {}
